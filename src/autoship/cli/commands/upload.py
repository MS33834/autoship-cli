...
def upload(
    ...
    if not dry_run and not yes and not typer.confirm(f"Upload to {target}?"):
        typer.echo("Aborted.")
    ...
    try:
        ...
        error = UploadError(f"Upload to {target} failed: {exc}")
        audit.record(
            "upload.error",
    ...
    if result.url:
        typer.echo(f"Uploaded to {target}: {result.url}")
    else:
        typer.echo(f"Uploaded to {target}")
