...
> 状态：P1-1～P1-5 ✅ | P2-1～P2-8 ✅ | P3-1～P3-8 ✅ | P4-1 ✅ | P4-2～P4-6：进行中  
...
- **状态**：已完成（CHANGELOG.md 自动生成已接入 Release 工作流）。
- **Owner**：发布/集成组
- **验收标准**：
...
- **实现**：
  - 新增 `scripts/release_changelog.py`：从 GitHub Release notes 生成 Keep a Changelog 版本节并幂等写入根 `CHANGELOG.md`（保留 `## [Unreleased]` 与版本顺序）。
  - `.github/workflows/release.yml` 新增 `update-changelog` job：tag push 后从 GitHub Release 取 notes，自动更新 `CHANGELOG.md` 并提交回 `main`。
  - `tests/test_release_changelog.py` 覆盖渲染、插入顺序、幂等、dry-run、非法版本号。
  - 三语 `docs/release-checklist.md` §1 已同步说明根 `CHANGELOG.md` 改为自动生成。
- **相关文件**：`CHANGELOG.md`、`.github/workflows/release.yml`、`scripts/release_changelog.py`、`tests/test_release_changelog.py`。
